...
SLIDES = [
    dict(kind="title", title="Can touch help the brain clear itself?",
         sub="Wearable haptic stimulation, brain rhythms, and glymphatic clearance — rationale, plan, and first electrophysiology (in the mouse)",
         footer="Pardis Miri  ·  a Snyder Lab (Stanford) × Buzsáki Lab (NYU) collaboration",
         notes="The program question: can a wearable, touch-based stimulus engage brain rhythms in a way that helps the brain clear waste (the glymphatic system), with Alzheimer's relevance? I'll motivate it from the literature, lay out what to test, then show a first electrophysiology study testing the premise. I separate 'plausible mechanism' from 'proven' throughout. ~35 min — please interrupt."),

    # ===================== PART 1 — VISION =====================
    dict(kind="section", title="Part 1 · The vision — clearing the brain through touch"),
    ...
    dict(kind="content", kicker="the thesis", title="The proposal: engage brain rhythms through touch to drive the brain's clearance system",
         take="Wearable haptic stimulation → entrain brain rhythms → engage glymphatic clearance of amyloid-β / tau. A mechanism hypothesis — explicitly not yet a treatment claim.",
    ...
    dict(kind="content", kicker="why touch", title="Why haptic, and why wearable: the white space next to light and sound",
         take="40 Hz light/sound (GENUS) is the developed approach but hard to wear daily. Touch is the open frontier — wearable, sleep-compatible, and pairs naturally with sensing.",
    ...
    dict(kind="content", kicker="evidence · haptic", title="The haptic anchor: 40 Hz vibration helps in mouse AD models, and cortex codes vibration frequency",
         take="40 Hz whole-body vibration reduces pathology and improves function in AD-model mice (Suk 2023). Somatosensory cortex faithfully encodes vibration frequency (Romo, Mountcastle) — touch can carry a frequency code.",
         notes="The touch-specific evidence. Suk 2023 (Tsai lab) showed 40 Hz whole-body vibration reduces pathology and improves motor function in mouse AD models — the central haptic anchor, though not yet a human AD trial. And classic somatosensory physiology (Romo, Mountcastle flutter coding) shows primary somatosensory cortex faithfully encodes vibration frequency — so a frequency-specific tactile drive is biologically grounded, not wishful."),
    dict(kind="content", kicker="→ what to test", title="So which haptic parameters should we test?",
    ...
    dict(kind="section", title="Part 3 · Does touch even reach the brain? A first study",
         sub="Foundational electrophysiology in the mouse — before any clearance claim"),
    dict(kind="content", title="Design: parametric haptic stimulation with simultaneous electrophysiology", fig=TRIAL,
    ...
    dict(kind="content", kicker="key references", title="Key references",
         ...
         notes="The five anchors: the origin of sensory-gamma (Tsai 2016); the gamma→glymphatic bridge (Murdock 2024); the haptic anchor (Suk 2023); the mechanism legitimizing brain-state stimulation of clearance (Jiang-Xie–Kipnis 2024); and the guardrail that keeps us honest (Buzsáki/Soula: a steady-state response is not entrainment). Full lit-review repository available on request."),

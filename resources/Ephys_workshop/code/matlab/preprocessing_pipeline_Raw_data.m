...
addpath(genpath('githubCodesFromEphysWorkshop'))
...
digiOn = []; digiOff = [];
digiOn = sort([digital_on{1,1}]); 
digiOff = sort([digital_off{1,1}]);
digiAll = sort([digiOn;digiOff]);

% Define number of trials and events per trial (must match your stimulus protocol)
events_per_trial = 5; % Adjust this value based on your protocol
...
Optomanipulation = [];
Optomanipulation.On = stimulation.startSample; 
Optomanipulation.Off = stimulation.endSample;
Optomanipulation.timestamps = reshape(stimulation.startTime,[],1);
Optomanipulation.timestamps(1:numel(stimulation.startTime),2) = reshape(stimulation.endTime,[],1);

% Define the stimulus parameters for analysis
% --- USER CUSTOMIZATION REQUIRED ---
Optomanipulation.amplitude = [2500, 2750, 3000, 3250, 3500]; % Example amplitudes (in your desired units)

% Calculate center time and duration
Optomanipulation.center = Optomanipulation.timestamps(:,1) + (Optomanipulation.timestamps(:,2) - Optomanipulation.timestamps(:,1))/2;
...
artefact(1:2:2*size(Optomanipulation.timestamps,1),1) = Optomanipulation.timestamps(:,1) - 0.001; % Start near pulse ON
artefact(2:2:2*size(Optomanipulation.timestamps,1),1) = Optomanipulation.timestamps(:,2) - 0.001; % Start near pulse OFF
